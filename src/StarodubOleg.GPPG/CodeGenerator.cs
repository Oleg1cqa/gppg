...
namespace QUT.GPGen
{
	internal class CodeGenerator
	{
		...
		internal void Generate(List<AutomatonState> states )
		{
			...
			//
			string outFile = Program.OutFileName;
			if (outFile == null)
			...
			if (Program.ShareTokens && grammar.DatFileName != null) // serialize Terminals dictionary.
			{
				try {
					...
					using (var fStrm = new FileStream(grammar.DatFileName, FileMode.Create))
					{
						var formatter = new JsonSerializer();
						formatter.Serialize(new StreamWriter(fStrm), grammar.terminals);
					}
				}
				catch (IOException x) {
			...
			//
			if (!Program.ImportedTokens) {
				if (Program.CsTokenFile)
			...
			if (tWrtr != null) {
				...
				tWrtr.Dispose(); // Close the optional token name stream
			}

			if (cstWrtr != null) {
				cstWrtr.Dispose(); // Close the optional token declaration stream
			}

			if (cWrtr != null)
			{
				cWrtr.WriteLine("// End conflict information for parser");
				cWrtr.Dispose(); // Close the optional token name stream
			}
		...
		private void GenerateCopyright() {
			...
			Console.WriteLine();
			if (Program.IncludeInfo) {
				Console.WriteLine( "// GPPG version " + Program.versionInfo );
				Console.WriteLine( "// Machine:  " + Environment.MachineName );
				Console.WriteLine( "// DateTime: " + DateTime.Now.ToString() );
				Console.WriteLine( "// Input file <{0}>", grammar.InputFileIdent );
			}

			if (!Program.NoFilename && !Program.IncludeInfo)
				Console.WriteLine( "// Input file <{0}>", grammar.InputFileIdent );

			Console.WriteLine();

			Console.Write( "// options:" );
			if (Program.Babel) Console.Write( " babel" );
			if (Program.Conflicts) Console.Write( " conflicts" );
			if (Program.Lines) Console.Write( " lines" ); else Console.Write( " no-lines" );
			if (Program.Diagnose)
				Console.Write( " diagnose & report" );
			else if (Program.Report)
				Console.Write( " report" );
			if (Program.Defines) Console.Write( " defines" );
			if (Program.ForGplex) Console.Write( " gplex" );
			if (Program.Conflicts) Console.Write( " conflicts" );
			if (Program.Listing) Console.Write( " listing" );
		...
		private static void GeneratedCodeAttribute() {
			Console.WriteLine( 
				"[GeneratedCodeAttribute( \"Gardens Point Parser Generator\", \"{0}\")]", Program.versionInfo );
		}
		...
		private void InsertCodeSpan( LexSpan span ) {
			if (span != null) {
				...
				if (Program.Lines) {
					Console.WriteLine( "#line {0} \"{1}\"", span.startLine, Program.LinesFilename ?? grammar.InputFilename );
