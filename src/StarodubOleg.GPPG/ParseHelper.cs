...
namespace QUT.GPGen.Parser
{
	internal partial class Parser
	{
		...
		private void TidyUpDefinitions(LexSpan def)
		{
			...
			if (Program.ShareTokens) {
				grammar.DatFileName = baseName + "Tokens.dat";
			}
			if (Program.Conflicts) grammar.DiagFileName = baseName + ".conflicts";
			...
			if (Program.ImportedTokens) {
				...
				try {
					using (var fStrm = new FileStream(grammar.DatFileName, FileMode.Open))
					{
						var formatter = new JsonSerializer();
						grammar.terminals = (Dictionary<string, Terminal>)formatter.Deserialize(new JsonTextReader(new StreamReader(fStrm)));
				...
				foreach (string str in list) {
					...
					switch (TokenOf( str )) {
						case Token.litchar: // This is a character literal symbol
							if (Program.ImportedTokens && Terminal.BumpsMax( str ))
