...
namespace QUT.GPGen {
	class Program
	{
		...
		private static int Main( string[] args ) {
			...
			var info = typeof(Program).GetTypeInfo().Assembly.GetCustomAttribute<AssemblyFileVersionAttribute>();
			...
			try {
				...
				if ( parser.Parse() && !handler.Errors ) {
					...
					if (Report || DoDiagnose) {
						...
						try {
							using (var htmlFile = new FileStream(htmlName, System.IO.FileMode.Create))
							{
		...
		private static void DisplayVersion() {
			object info = typeof(Program).GetTypeInfo().Assembly.GetCustomAttribute<AssemblyFileVersionAttribute>();
